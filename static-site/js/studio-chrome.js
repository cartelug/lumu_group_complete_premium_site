/* ============================================================
   LUMU · v2 STUDIO — shared chrome
   Injects the sprite, topbar, header, mobile menu, footer,
   fixed helpers on every v2 page. Runs before studio.js.
   Edit NAV / PHONE / EMAIL / ADDRESS in one place here.
   ============================================================ */
(function () {
  "use strict";
  ...
  var navHtml = NAV.map(function (n) {
    return '<a href="' + n[0] + '"' +
           (n[2] === page ? ' class="active" aria-current="page"' : "") +
           ">" + n[1] + "</a>";
  }).join("");

  var mmHtml = NAV.map(function (n, idx) {
    return '<a class="mm-link' + (n[2] === page ? " active" : "") + '"' +
           (n[2] === page ? ' aria-current="page"' : "") +
           ' style="--d:' + (0.06 + idx * 0.05).toFixed(2) + 's" href="' + n[0] +
           '"><i>' + n[3] + "</i>" + n[1] + "</a>";
  }).join("");

  /* --- brand mark: an editorial serif L in ink --- */
  var logo = function (id, sub) {
    return '<svg class="logo-mark" viewBox="0 0 44 44" aria-hidden="true">' +
      '<rect width="44" height="44" rx="9" fill="none" stroke="#1a1613" stroke-width="1.4"/>' +
      '<text x="22" y="31" text-anchor="middle" font-family="Fraunces, serif" font-size="26" font-style="italic" font-weight="400" fill="#1a1613">L</text>' +
      '<circle cx="34" cy="10" r="2.4" fill="#e07a1c"/>' +
      '</svg>' +
      '<span class="brandtext">Lumu <b>Autodealers</b><small>' + sub + "</small></span>";
  };

  /* --- icon sprite (unchanged from v1 — same icon vocabulary) --- */
  var sprite =
  ...
  var top =
    ...
    '<div class="topbar"><div class="container topbar-row">' +
      '<div class="topbar-contact"><a href="' + TEL + '">' + PHONE + '</a>' +
      '<span class="topbar-sep"></span><a href="mailto:' + EMAIL + '">' + EMAIL + "</a></div>" +
      '<div class="topbar-meta"><span>' + ADDRESS + '</span>' +
      '<span class="topbar-sep"></span><span>Mon – Sat · 8:00 – 18:00</span></div>' +
    "</div></div>" +
    '<header class="header"><div class="container nav">' +
      '<a class="brand" href="index.html" aria-label="Lumu Autodealers home">' + logo("lm", "& Logistics · Kampala") + "</a>" +
      '<nav class="nav-links" aria-label="Primary">' + navHtml + "</nav>" +
      '<div class="nav-actions">' +
        '<a class="btn btn-primary" href="contact.html#book">Open a job card</a>' +
  ...
  var bottom =
    '<footer class="footer"><div class="container"><div class="footer-grid">' +
      '<div><a class="brand" href="index.html" aria-label="Lumu Autodealers home">' + logo("lf", "Built on Trust") + "</a>" +
        '<p class="mt-24">Motor vehicle repairs, servicing, computerized diagnostics, genuine spare parts and fleet logistics in Kampala since 2018.</p>' +
        '<div class="cta-row mt-24"><a class="btn btn-primary" href="contact.html#book">Open a job card</a></div>' +
      '</div>' +
      '<div class="f-col"><h3>Workshop</h3>' +
        '<a href="vehicles.html">Vehicles for sale</a>' +
        '<a href="services.html">All services</a>' +
        '<a href="services.html#bay-2">Diagnostics</a>' +
        '<a href="services.html#bay-5">Spare parts</a>' +
        '<a href="fleet.html">Fleet &amp; logistics</a>' +
      '</div>' +
      '<div class="f-col"><h3>Company</h3>' +
        '<a href="about.html">About Lumu</a>' +
        '<a href="real-estate.html">Lumu Real Estate</a>' +
        '<a href="contact.html">Contact</a>' +
        '<a href="contact.html#book">Open a job card</a>' +
      '</div>' +
      '<div class="f-col f-contact"><h3>Find us</h3>' +
        '<p class="mono" style="font-size:.8rem;line-height:2">P.O. Box 72434, Kampala<br>' + ADDRESS + "</p>" +
        '<a href="' + TEL + '">' + PHONE + '</a>' +
        '<a href="tel:+256782493499">' + PHONE2 + '</a>' +
        '<a href="mailto:' + EMAIL + '">' + EMAIL + "</a>" +
      '</div>' +
    '</div>' +
    '<div class="bottom">' +
      '<span>© ' + year + ' Lumu Autodealers &amp; Logistics Ltd</span>' +
      '<span>Built on Trust</span>' +
    '</div>' +
    '<div class="f-word" aria-hidden="true">Lumu <b>Autodealers</b></div>' +
    "</div></footer>" +
    '<div class="mobile-actions">' +
      '<a href="' + TEL + '"><svg class="ic" aria-hidden="true"><use href="#ic-phone"/></svg>Call</a>' +
      ...
      '<a class="primary" href="contact.html#book"><svg class="ic" aria-hidden="true"><use href="#ic-file"/></svg>Book</a>' +
    '</div>' +
    '<a class="wa-fab" href="' + WA + encodeURIComponent("Hello Lumu Autodealers, I have a question.") + '" target="_blank" rel="noopener" aria-label="Chat on WhatsApp">' +
